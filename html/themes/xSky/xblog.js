...
function post_list(pageid)
{
    ...
    $.ajax({
        ...
        for(var i = 0; i<data.length; i++) {
            ...
            html += '<H2><A  title="'+it.post_title+'" href="javascript:onPost('+it.ID+', \'\');" rel="bookmark">'+it.post_title+'</A></H2>';
...
function onpInfopage(id)
{
    console.log('onpInfopage '+id);
    var url  = "/post?id="+id;
    $.ajax({
        type: "get",
        url: url,
...
function infopage()
{
    ...
    $.ajax({
        ...
        for(var i = 0; i<data.length; i++) {
            var it = data[i];
            html += '<li><a href="javascript:onpInfopage('+it.post_id+')";>'+it.page_title+'</a></li>';
            $('#widget_infopage').html(html);
        }  
    }
    });
}
...
function links()
{
    ...
    $.ajax({
        ...
        for(var i = 0; i<data.length; i++) {
            var it = data[i];
            html = '<li><a href="'+it.link_url+'">'+it.link_name+'</a></li>';
...
function postview()
{
    ...
    $.ajax({
        ...
        for(var i = 0; i<data.length; i++) {
            var it = data[i];
            html = '<li><a href="javascript:onPost('+it.ID+',\'\');">'+it.post_title+'</a></li>';
