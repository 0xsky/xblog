...
jQuery(function(){
    ...
    $("#BTpostRevert").live("click",function(){
        var updateURL = "/admin_post?action=revert";
    ...
    $("#BTpostDelete").live("click",function(){
        var updateURL = "/admin_post?action=delete";
        var id = $(this).parents("tr").find(".id").text();
        $.ajax({
            type: "POST",
            url: updateURL,
...
function GetLinkList(){
    $.get("/admin_links?action=view&page="+post_pageid,function(data, textStatus) {
            $("#links").html("");
            var n = data.length;
            var strhtml = "<table><tr><td>id</td><td>链接地址</td><td>显示</td><td>操作</td><td>操作</td></tr>";
            
            $.each(data ,function(i){
                var js = data[i];
                strhtml += "<tr><td class='id'>"+js.link_id+"</td>";
                strhtml += "<td><input class='url' size='40' type='text'  value='"+js.link_url+"' /></td>";
                strhtml += "<td><input class='comment' size='40' type='text'  value='"+js.link_name+"' /></td>";
            ...
            strhtml+="<br><br><br><p><hr>添加链接地址";
            var strhtml = "<hr><table><tr><td>id</td><td>链接地址</td><td>显示</td><td>操作</td></tr>";
...
function GetCatalogList(){
    $.get("/admin_catalog?action=view",function(data, textStatus) {
        $("#catalogset").html("");
        var n = data.length;
        ...
        $.each(data ,function(i){
            var js = data[i];
            strhtml += "<tr><td class='id'>"+js.ID+"</td>";
            
            strhtml += "<td><input class='catalogid' size='1' type='text'  value='"+js.classify_id+"' /></td>";
            strhtml += "<td><input class='catalogname' size='40' type='text'  value='"+js.classify_name+"' /></td>";
            strhtml += "<td><input class='catalogstatus' size='1' type='text'  value='"+js.classify_status+"' /></td>";
            strhtml += "<td><input type='submit' value='update' id='BTcatalogupdate'></td>";
            max_id = parseInt(js.ID) +1;
        });
        strhtml+="</table>";
...
function GetCatalogSelect(){
    $.get("/admin_catalog?action=view",function(data, textStatus) {
        ...
        $.each(data ,function(i){
            var js = data[i];
            strhtml += "<option value='"+ js.classify_id +"'>"+ js.classify_name+"</option>";
        });
        strhtml+="</select>";
...
function GetPostList(){
        $.get("/admin_post?action=view&page="+post_pageid,function(data, textStatus) {
                $("#posttotal").html("");
                $("#postlist").html("");
                var n = data.length;
                var Total = "文章总数:" + n;
                ...
                $.each(data ,function(i){
                    var js = data[i];
                    strhtml += "<tr><td class='id'>"+js.ID+"</td>";
                    strhtml += "<td>"+js.post_classify+"</td>";
                    strhtml += "<td>"+js.post_author+"</td>";
                    strhtml += "<td>"+js.post_date+"</td>";
                    strhtml += "<td id='list_title'>"+js.post_title+"</td>";
                    strhtml += "<td>"+js.post_status+"</td>";
                    strhtml += "<td><a href='javascript:onEditPost("+js.ID+");'>[edit]</a></td>";
...
{
    var post_classify_id = 0;
    var postlist_h = $("#postlist").outerHeight(); 
    ...
    $.get("/admin_catalog?action=view",function(data, textStatus) {
        $("#post_edit_select_classify").html("");
        var strhtml = "分类: <select name='classify'>";
        $.each(data ,function(i){
            var js = data[i];
            if(js.classify_id==post_classify_id) {
                strhtml += "<option selected = 'selected' value='"+ js.classify_id +"'>"+ js.classify_name+"</option>";
            } else{
                strhtml += "<option value='"+ js.classify_id +"'>"+ js.classify_name+"</option>";
            }
        });
        strhtml+="</select>";
        $("#post_edit_select_classify").append(strhtml);
    });
...
function update_post(){ 
    ...
    $.ajax({
        type: "POST",
        url: "/admin_post?action=update_post",
...
function GetWebConfig(){
    $.get("/admin_config?action=view&page="+post_pageid,function(data, textStatus) {
        $("#webconfig").html("");
        var n = data.length;
        var strhtml = "<table><tr><td>name</td><td>value</td><td>method</td></tr>";
        var config = data[0];
